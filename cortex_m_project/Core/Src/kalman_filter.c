...
int main(void)
{
  ...
  // 100ms 지연(5번 주기)을 카운트하기 위한 정적 변수 선언
  ...
  while (1)
  {
    // 5번의 루프(20ms * 5 = 100ms)마다 한 번씩만 새로운 AI(더미) 타겟 좌표를 가져옴
    ...
    // 카운터를 0부터 4까지 반복 (0, 1, 2, 3, 4 -> 총 5스텝 = 100ms)
    ai_delay_counter++;
    ...
    Motor_PID_Process_With_Error(&pan_pid, &tilt_pid, cached_target_x, cached_target_y, &htim2);
