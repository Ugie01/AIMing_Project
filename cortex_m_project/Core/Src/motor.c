...
void Motor_PID_Process_With_Error(PID_Controller *pan, PID_Controller *tilt, float target_x, float target_y, TIM_HandleTypeDef *htim) {
    ...
    // 범위 제한 안전장치
    if (target_pan_ccr > ANGLE_MAX) target_pan_ccr = ANGLE_MAX;
    if (target_pan_ccr < ANGLE_MIN) target_pan_ccr = ANGLE_MIN;
    
    if (target_tilt_ccr > ANGLE_MAX) target_tilt_ccr = ANGLE_MAX;
    if (target_tilt_ccr < ANGLE_MIN) target_tilt_ccr = ANGLE_MIN;
