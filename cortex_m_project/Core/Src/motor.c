...
void Motor_PID_Process_With_Error(PID_Controller *pan, PID_Controller *tilt,
	...
	if (pan->current > ANGLE_MAX)
		pan->current = ANGLE_MAX;
	if (pan->current < ANGLE_MIN)
		pan->current = ANGLE_MIN;

	if (tilt->current > ANGLE_MAX)
		tilt->current = ANGLE_MAX;
	if (tilt->current < ANGLE_MIN)
		tilt->current = ANGLE_MIN;
