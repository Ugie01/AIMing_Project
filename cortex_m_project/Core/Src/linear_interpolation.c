...
int main(void)
{
  ...
  // 지연되는 동안 사용할 마지막 타겟 좌표를 기억할 변수
  static float cached_target_x = 50.0f;
  static float cached_target_y = 50.0f;
  
  // --- 선형 보간을 위한 실시간 상태 변수들 ---
  static float current_x = 50.0f;
  static float current_y = 50.0f;
  static float target_x_old = 50.0f;
  static float target_y_old = 50.0f;
  static float target_x_new = 50.0f;
  static float target_y_new = 50.0f;

  static uint32_t ticks_since_last_update = 0;
  static uint32_t estimated_interval = 5; // 초기 갱신 간격 가정 (약 100ms)
  static uint8_t ai_delay_counter = 0;
  int random_num;
  ...
  while (1)
  {
    // 1. 새로운 AI 좌표가 들어오는 시점 (ai_delay_counter == 0)
    if (ai_delay_counter == 0)
    {
        // 직전 텀까지 걸린 실제 틱 간격을 보간 간격으로 설정
        estimated_interval = ticks_since_last_update;
        if (estimated_interval < 1) estimated_interval = 1; 
        ticks_since_last_update = 0;

        // [중요] 이전 목표를 시작점(old)으로 삼고, 현재 보간 중이던 위치를 출발점으로 고정
        target_x_old = current_x;
        target_y_old = current_y;

        // 새로운 AI 목표 수신
        Get_Dummy_Target_Coords(&target_x, &target_y);
        target_x_new = target_x;
        target_y_new = target_y;

        // 다음 갱신까지 대기할 랜덤 틱 수 설정 (예: 2 ~ 6 틱)
        random_num = rand() % (6 - 2 + 1) + 2;
    }
    ...
    // 3. 현재 갱신 주기(estimated_interval) 안에서 몇 번째 스텝인지에 따른 비율(Alpha) 계산
    // ai_delay_counter를 이용해 0부터 1까지 점진적으로 증가하는 비율 계산
    float alpha = (float)ai_delay_counter / (float)estimated_interval;
    if (alpha > 1.0f) alpha = 1.0f;

    // 4. 선형 보간(LERP) 공식 적용: 시작점(old)에서 새로운 목표(new)까지 부드럽게 이동
    current_x = target_x_old + (target_x_new - target_x_old) * alpha;
    current_y = target_y_old + (target_y_new - target_y_old) * alpha;

    // 5. 모니터링 출력
    printf("Target(%.1f, %.1f) | Interp(%.1f, %.1f) | Pan_Cur: %d | Tilt_Cur: %d\r\n", 
           target_x_new, target_y_new, 
           current_x, current_y, 
           (int)pan_pid.current, (int)tilt_pid.current);

    // 6. PID 제어기에 부드럽게 보간된 좌표 투입
