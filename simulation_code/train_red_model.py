...
def create_or_verify_dataset(dataset_dir="./synthetic_red_dataset", num_samples=2500, img_size=95):
    ...
    for i in range(num_samples):
        ...
        # 30% 확률로 빨간색 타겟이 없는 순수 배경(Negative) 샘플 생성
        ...
        if has_target > 0.0:
            ...
            # 💡 빨간색 타겟 강조 색상 생성 (R 채널 높임)
            red_color = (
                random.randint(150, 255),  # R (빨간색 강조)
            ...
            if random.random() > 0.5:
                ...
                draw.ellipse([x1, y1, x2, y2], fill=red_color)
...
def main():
    # 💡 데이터셋 폴더명을 빨간색용으로 변경
    dataset_dir = "./synthetic_red_dataset"
    img_size = 95
    ...
    # 💡 저장되는 모델 파일명을 red_detector.tflite로 변경
    tflite_path = "red_detector.tflite"
