...
def main():
    ...
    if EXPERIMENT_MODE == "DEAD_ZONE":
        ...
        target_speed_options = [3, 5, 10, 20, 40]
    ...
    if USE_GUI:
        ...
        cv2.createTrackbar("KP x10000", debug_window_name, 30, 100, nothing)
        cv2.createTrackbar("KI x10000", debug_window_name, 1, 50, nothing)
        cv2.createTrackbar("KD x100000", debug_window_name, 2, 50, nothing)
        ...
        while running:
            ...
            if current_time - last_control_time >= CONTROL_PERIOD:
                ...
                if is_locked_on or episode_step >= MAX_EPISODE_STEPS:
                    ...
                    in_frame_ratio = (
                        (in_frame_count / ai_frame_count) * 100.0 if ai_frame_count > 0 else 0.0
                    )

                    csv_writer.writerow([trial_id, current_deadzone, target_size_scale, target_speed_val, target_distance, success, time_to_lock, hit_ratio, in_frame_ratio])
                    csv_file.flush()
                    print(f"📊 [Trial {trial_id}/{max_total_trials}] Deadzone: {current_deadzone}px | Success: {success} | Time: {time_to_lock:.2f}s | In-Frame:"
      f" {in_frame_ratio:.1f}%")
