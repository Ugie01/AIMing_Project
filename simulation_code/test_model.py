...
def main():
    ...
    test_img_dir = "./synthetic_green_dataset/test/images"
    test_lbl_dir = "./synthetic_green_dataset/test/labels"
    ...
    for i, idx in enumerate(sample_indices):
        ...
        if true_conf > 0.5:
            plt.scatter([true_x], [true_y], color='green', marker='o', s=30, label='True')
        if conf > 0.5:
            plt.scatter([pred_x], [pred_y], color='red', marker='x', s=30, label='Pred')
            
        plt.title(f"#{i+1} Conf:{conf:.2f}", fontsize=8)
        plt.axis('off')
        
    plt.suptitle("TFLite Test Results with Confidence", fontsize=14)
    plt.tight_layout()
    plt.show()
